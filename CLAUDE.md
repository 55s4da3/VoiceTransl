...
## 当前状态
- 分支 `feat/qwen-streaming-integration`；上游整合及 OpenCode Go/DeepSeek V4.1 Flash 适配均已提交，并按用户要求推送到 `origin` 同名分支。
...
- 回归基线：仓库 `.venv` 执行 158 项测试全部通过，1 项依赖完整本地 CrispASR 二进制/模型/音频夹具的集成测试跳过；GitHub Actions 构建 YAML 可正常解析。
